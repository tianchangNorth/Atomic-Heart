      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center space-x-3 mb-4">
          <Avatar class="h-8 w-8">
            <!-- <AvatarImage :src="repository.owner.avatar_url" :alt="repository?.owner?.login ?? 'tianchang'" /> -->
            <AvatarFallback>{{ "T" }}</AvatarFallback>
          </Avatar>
          <div>
            <div class="font-medium text-foreground">{{ repository?.owner?.login ?? 'tianchang' }}</div>
            <div class="text-sm text-muted-foreground">{{ repository?.owner?.type ?? 'User' }}</div>
          </div>
        </div>
